...
def generate_bash_script(llm, user_command, current_stage, previous_script_output):
    ...
    You are a command interpreter designed to assist a system administrator who is not familiar with Bash scripting.
...
def parse_gemini_response(response_text):
    response_types = [
        ...
        ("question", r"REQUEST_UNCLEAR\n(.*)"),
        ("answer", r"FINAL_ANSWER\n(.*)"),
    ]

    for response_type, regex in response_types:
        match = re.search(regex, response_text, re.DOTALL)
        if match:
            return response_type, match.group(1)
